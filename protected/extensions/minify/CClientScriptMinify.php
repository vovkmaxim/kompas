...
class CClientScriptMinify extends PEClientScript
{
	...
	public function render(&$output)
	{
		...
		if($this->_minifyGroupsUpdated){
            ...
            @chmod(Yii::app()->getRuntimePath()."/minifyGroups.var", 0666);
        }
	}
		...
		if(!empty($this->cssFiles)){
			foreach ($this->cssFiles as $url => $media) {
				if (strpos($url, $webroot)!==FALSE) {
					...
					$url = str_replace($webroot . $webroot, $webroot, $url);
	...
	public function absolutizeURL($url){
        $dir = dirname(Yii::app()->request->getScriptFile());
        $url = str_replace($dir, '', $url);
		return $dir.str_replace(Yii::app()->request->getBaseUrl(), '', $url);
