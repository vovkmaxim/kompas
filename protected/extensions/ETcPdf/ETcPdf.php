 ...
 * if needed.
 */
require_once(dirname(__FILE__).'/tcpdf/customTCPDF.php');
...
class ETcPdf
{
   ...
   public function __construct($orientation, $unit, $format, $unicode, $encoding)
   {
      ...
      if (!is_bool($unicode))
        ...
        define ("K_PATH_CACHE", Yii::app()->getRuntimePath().DIRECTORY_SEPARATOR);
        define ("K_PATH_IMAGES", Yii::getPathOfAlias('webroot').DIRECTORY_SEPARATOR."upload");
        define ("PDF_HEADER_LOGO", '/backend/images/logo.png');

		$this->myTCPDF = new customTCPDF($orientation, $unit, $format, $unicode, $encoding);
   }
    ...
    */
	...
	{
		if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='customTCPDF') return call_user_func_array(array($this->myTCPDF, $method), $params);
		else throw new CException(Yii::t('ETcPdf', 'Can not call a method of a non existent object'));
	}

	public function __set($name, $value)
	{
	   if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='customTCPDF') $this->myTCPDF->$name = $value;
	   else throw new CException(Yii::t('ETcPdf', 'Can not set a property of a non existent object'));
	}

	public function __get($name)
	{
	   if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='customTCPDF') return $this->myTCPDF->$name;
