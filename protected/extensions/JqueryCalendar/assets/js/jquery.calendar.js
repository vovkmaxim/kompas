...
(function($){
	...
	var defaults = {
        ...
        allowedit	    : false,		// Enable or disable edit.
	...
	var _private = {
		...
		event : {
            ...
            updateallnote: function( e, speed, ease ){
                ...
                if( data && values ){
                    ...
                    // Run the on select handler, if the user has defined one for this instance.
                    var veto = data.settings.eventeditevent.apply(values.calendar,[values.uid,values,$event]);

                    // Check whether the eventeditevent handler overrode the selection.
                    if( veto === undefined || veto ){
			...
			week : {
				...
				event : function( data, values )
				{
					...
					if( !( 'elems' in values ) || values.elems.length < 1 ){
						...
						if( !data.settings.allowedit ) $event.find('span.button-edit').hide();
					...
					if( data && values ){
						...
						$events.each(function( i, event ){
							...
							if( selected ){
								...
								$event.find('span.button-edit').show();

								if( spanning && i==0 ){
									$event.find('span.button-remove').hide();
									$event.find('span.button-edit').hide();
								}
								if( i > 0 ){
	...
	var fragments = {
		...
		event : $([
			...
			.find('span.button-edit').bind('click.'+plugin_name,_private.event.updateallnote).end(),
	...
	var methods = {
		...
		select : function( uid, speed, ease ){
			...
			if( data ){
				...
				// Find the event element.
				var $event = data.elements.container.find('div.ui-'+plugin_name+'-event[data-id="'+uid+'"]');
			...
			if( data ){
				...
				if( typeof value == 'undefined' ){
					...
					switch( key ){
						...
						break;
                        case 'allowedit' :
							data.elements.container.find('div.ui-'+plugin_name+'-event span.button-edit').toggle(Boolean(value));
						break;
						case 'allowresize' : 
